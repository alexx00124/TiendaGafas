...
function* profileSaga({ type, payload }) {
  try {
    switch (type) {
      case UPDATE_EMAIL:
        yield handleUpdateEmail(payload);
        break;
      case UPDATE_PROFILE:
        yield handleUpdateProfile(payload);
        break;
      default:
    ...
    yield put(setLoading(false));
    if (type === UPDATE_PROFILE && e.code === 'auth/wrong-password') {
      yield call(displayActionMessage, 'Wrong password, profile update failed :(', 'error');
    } else {
      yield call(displayActionMessage, `:( Failed to update profile. ${e.message || ''}`, 'error');
    }
  }
}
